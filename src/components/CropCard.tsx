...
function CropCard({ crop }: CropCardProps) {
  ...
  const getSuitabilityLabel = (score: number) => {
    if (score >= 80) return t("cropCard.highlySuitable");
    if (score >= 60) return t("cropCard.suitable");
    if (score >= 40) return t("cropCard.moderatelySuitable");
    return t("cropCard.notSuitable");
  };
  ...
  const getCropEmoji = (name: string) => {
    ...
    if (lowerName.includes("groundnut") || lowerName.includes("peanut"))
      return "🥜";
    if (lowerName.includes("cassava")) return "🍠";
    if (lowerName.includes("banana")) return "🍌";
    if (lowerName.includes("mango")) return "🥭";
    if (lowerName.includes("orange") || lowerName.includes("citrus"))
      return "🍊";
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-lg font-bold">
              {language === "ny" ? crop.name_chichewa : crop.name}
            </h3>
            <p className="text-sm opacity-75">
              {language === "ny" ? crop.name : crop.name_chichewa}
            </p>
          </div>
        </div>
        {/* Score badge */}
      ...
      <p className="font-semibold mb-3">
        {getSuitabilityLabel(crop.suitability_score)}
      </p>

      {/* Requirements section */}
      <div className="border-t pt-3 mt-3 text-sm space-y-1 opacity-80">
        <p>
          <span className="font-medium">{t("cropCard.phRange")}:</span>{" "}
          {crop.soil_requirements.min_ph.toFixed(1)} -{" "}
          {crop.soil_requirements.max_ph.toFixed(1)}
        </p>
        <p>
          <span className="font-medium">{t("cropCard.elevation")}:</span>{" "}
          {crop.elevation_requirements.min_elevation}m -{" "}
